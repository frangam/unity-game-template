...
//		if(!isInited)
			init();
//		Debug.Log("OnEnable - initing picker");
	}
...
//			init();
//
//		Debug.Log("Start - initing picker");
//	}

	public void init(){
		int width = (int)hsvSlider.rectTransform.rect.width;
		int height = (int)hsvSlider.rectTransform.rect.height;
		bool validDimensions = width > 0 && height > 0;

		GTDebug.log("Picker dimensions. Width: "+ width +", Height: "+height+". Valid dimensions ? "+validDimensions);

		if(validDimensions){
//			Debug.Log("init - valid dimensions");
			hsvSlider.texture = HSVUtil.GenerateHSVTexture(width, height);
    ...
    public void AssignColor(Color color)
    {
		...
		if(hsvSlider.texture != null && isInited){
