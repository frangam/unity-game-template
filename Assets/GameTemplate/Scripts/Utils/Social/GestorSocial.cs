...
public class GestorSocial : Singleton<GestorSocial> {
	//--------------------------------------
	// Atributos de configuracion
	//--------------------------------------
	[SerializeField]
	private UIPanel panelCaptura;
	...
	void Awake(){
		...
		urlIcono = Configuration.LINK_ICONO_APP;
...
//
	void Start(){
		...
		if (Configuration.USE_FACEBOOK && FB.IsLoggedIn) {
	...
	private void OnInitFB() {
		LoadManager.Instance.FbInited = true;
...
//		SA_StatusBar.text = "Loading friends..";
	...
	public void SubmitScore() {
		SPFacebook.instance.SubmitScore(GameManager.score);
	}
	...
	private void postear(SocialNetwork red, bool captura = false){
		if(captura){
			...
			if(red == SocialNetwork.TWITTER)
				...
				IOSSocialManager.instance.FacebookPost(mensaje+" "+Configuration.LINK_IOS_APP);
			#endif
		}
	}
